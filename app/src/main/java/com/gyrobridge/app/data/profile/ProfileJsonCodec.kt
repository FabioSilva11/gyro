...
object ProfileJsonCodec {
    ...
    fun decode(root: JSONObject): ControlProfile {
        ...
        val movement = root.optJSONObject("physicalMovement") ?: JSONObject()
        ...
        return ControlProfile(
            ...
            physicalMovement = PhysicalMovementConfig(
                ...
                zone = MovementZone(
                    ...
                    radius = movement.float("radius", j.float("radius", .12f)),
                    mappedDisplayRotation = movement.nullableEnum("mappedDisplayRotation"),
                ),
            ),
    ...
    private fun zoneJson(z: ScreenZone) = JSONObject().apply { put("centerX", z.centerX); put("centerY", z.centerY); put("width", z.width); put("height", z.height); put("boundaryMargin", z.boundaryMargin); put("autoRecenterThreshold", z.autoRecenterThreshold); put("mappedDisplayRotation", z.mappedDisplayRotation?.name) }
    private fun movementJson(config: PhysicalMovementConfig) = JSONObject().apply {
    ...
    private inline fun <reified T : Enum<T>> JSONObject.nullableEnum(key: String): T? = if (!has(key) || isNull(key)) null else runCatching { enumValueOf<T>(getString(key)) }.getOrNull()
